...
export default function BalesPage() {
  ...
  async function changeArchive() {
    if (!window.confirm(bale.isArchived ? '¿Reactivar esta paca? Sus piezas volverán al inventario.' : '¿Archivar esta paca? Sus piezas dejarán de estar disponibles. Se conservarán ventas, pagos, gastos e historial.')) return
    setSaving(true)
    setMessage('')
    try {
      await archiveBale(bale.id, !bale.isArchived)
      setParams(bale.isArchived ? { paca: bale.id } : {})
    } catch (error) { setMessage(error.message || 'No fue posible cambiar el archivo de la paca.') }
    finally { setSaving(false) }
  }

  return <div>
    <PageHeader eyebrow="Compras e inversión" title="Pacas" description="Inversión, recuperación y resultado individual. Archivar nunca borra la historia." />
    <div className="page-content max-w-5xl space-y-5 py-5 md:py-8">
      <div className="flex flex-wrap gap-3">
        <Link to="/pacas/nueva" className="inline-flex min-h-12 items-center gap-2 rounded-2xl bg-brand-900 px-5 text-sm font-bold text-white"><PackagePlus size={19} />Registrar una paca</Link>
      ...
      <nav className="flex gap-2" aria-label="Estado de pacas">
        <Link to="/pacas" className={tabClass(!archived)}>Vigentes ({data.bales.filter((item) => !item.isArchived).length})</Link>
        <Link to="/pacas?archivo=1" className={tabClass(archived)}>Archivadas ({data.bales.filter((item) => item.isArchived).length})</Link>
      </nav>
      {message && <p role="alert" className="rounded-xl bg-red-50 p-4 text-sm text-red-700">{message}</p>}
      {!bale ? <p className="rounded-2xl bg-white p-5 text-sm text-slate-500">{isLoading ? 'Cargando pacas…' : archived ? 'No hay pacas archivadas.' : 'No hay pacas vigentes. Registra una nueva o reactiva una desde Archivadas.'}</p> : <>
        <label className="block max-w-md text-sm font-bold text-slate-700">Paca seleccionada
        ...
        <article className="overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-100">
          <div className="flex flex-wrap items-center justify-between gap-3 bg-brand-900 p-5 text-white">
            ...
            {!bale.isArchived && <Link to={'/pacas/' + bale.id + '/editar'} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-white px-4 text-sm font-bold text-brand-900"><Pencil size={15} />Editar paca</Link>}
          </div>
          {bale.isArchived && <p className="bg-slate-100 px-5 py-3 text-sm text-slate-600">Archivada el {formatShortDate(bale.archivedAt)}. Sus piezas no se ofrecen en ventas nuevas. {bale.archivedReason}</p>}
          <dl className="divide-y divide-slate-100 px-5">
            ...
            <Detail label="Inversión de esta paca" value={formatCurrency(finances.investment)} />
            <Detail label="Ventas registradas de prendas" value={formatCurrency(finances.revenue)} />
            <Detail label="Cobrado de prendas" value={formatCurrency(finances.collected)} />
            <Detail label="Por cobrar de prendas" value={formatCurrency(finances.pending)} />
            <Detail label="Inversión pendiente de recuperar (con cobros)" value={formatCurrency(finances.investmentRemaining)} />
            <Detail label="Gastos operativos asignados a esta paca" value={formatCurrency(finances.operatingExpenses)} />
            ...
            <Detail label="Resultado de caja asignado a la paca" value={formatCurrency(finances.cashResult)} warning={finances.cashResult < 0} />
            ...
            {bale.availablePieces > 0 && <Detail label="Precio promedio necesario por pieza restante" value={formatCurrency(finances.requiredAveragePrice)} />}
          ...
          <p className="px-5 py-4 text-xs leading-5 text-slate-500">Los cobros de pedidos con varias pacas se distribuyen proporcionalmente entre prendas y delivery. No es el efectivo físico: también puede incluir transferencias. El resultado final descuenta la inversión completa y los gastos asignados; los gastos generales se descuentan en el resumen mensual.</p>
        </article>
        <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
          <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">{[['Recibidas', bale.receivedPieces], ['Vendidas', bale.soldPieces], ['Disponibles', bale.availablePieces], ['Dañadas', bale.damagedPieces]].map(([label, count]) => <div key={label}><b className="text-xl text-slate-900">{count}</b><p className="text-xs text-slate-500">{label}</p></div>)}</div>
        </article>
        <article className="overflow-hidden rounded-3xl bg-white shadow-soft ring-1 ring-slate-100">
