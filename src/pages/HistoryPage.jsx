...
export default function HistoryPage() {
  ...
  const actionNames = { archived: `En archivo: ${terms.purchaseSingular}`, reactivated: `Reactivación: ${terms.purchaseSingular}`, order_corrected: 'Pedido corregido', expense_corrected: 'Gasto corregido' }
  ...
  return <div>
    ...
    <div className="page-content max-w-5xl space-y-4 py-5 md:py-8">
      <label className="block max-w-sm text-sm font-bold text-slate-700">Mostrar<select value={type} onChange={(event) => { setType(event.target.value); setPage(0) }} className="sale-input mt-2"><option value="all">Todos los cambios</option><option value="bale">{terms.purchasePlural}</option><option value="sale">Pedidos</option><option value="expense">Gastos</option></select></label>
      <p className="text-xs text-slate-500">El historial comienza al instalar esta actualización. Las ventas y {terms.purchasePluralLower} anteriores siguen disponibles en sus pantallas.</p>
      ...
      {loading ? <p className="text-sm text-slate-500">Cargando historial…</p> : rows.map((row) => <article key={row.id} className="rounded-3xl bg-white p-5 ring-1 ring-slate-100">
        ...
        <Link to={row.entity_type === 'expense' ? '/gastos' : row.entity_type === 'bale' ? '/pacas?paca=' + row.entity_id : '/ventas?venta=' + row.entity_id} className="mt-3 inline-block text-sm font-bold text-brand-800 underline">Ver {row.entity_type === 'expense' ? 'gastos' : row.entity_type === 'bale' ? terms.purchaseSingularLower : 'pedido'}</Link>
      </article>)}
...
function Snapshot({ title, state }) {
  ...
  return <section className="rounded-2xl bg-slate-50 p-4"><h3 className="text-xs font-extrabold text-slate-500">{title}</h3><p className="mt-1 font-bold text-slate-900">Total: {formatCurrency(order?.total)}</p><p className="text-xs text-slate-500">{order?.fulfillment_method === 'delivery' ? 'Envío' : 'Retiro en punto de venta'} · pagado: {formatCurrency(order?.paid_amount)}</p><ul className="mt-3 space-y-2 text-xs text-slate-600">{state?.items?.map((item) => <li key={item.id}>{item.category_name ?? 'Artículo'} · {item.quantity} × {formatCurrency(item.unit_price)} · {item.allocations?.map((allocation) => allocation.bale_code).join(', ')}</li>)}</ul></section>
}
