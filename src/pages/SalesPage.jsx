    ...
    <div>
      ...
      <div className="page-content space-y-5 py-5 md:py-8">
        ...
        <section aria-labelledby="sales-list-title" className="max-w-5xl">
          <label className="mb-4 block text-sm font-bold text-slate-700">Paca de los pedidos
            <select className="sale-input mt-2 max-w-md" value={baleId ?? ''} onChange={(event) => {
              ...
              <option value="">Todas las pacas</option>
              {baleId && !selectedBale && <option value={baleId}>Paca no disponible</option>}
              {data.bales.map((bale) => <option key={bale.id} value={bale.id}>{bale.code}{bale.isArchived ? ' · archivada' : ''}</option>)}
            </select>
          </label>
          <nav className="mb-4 flex gap-2" aria-label="Archivo de pedidos">
            ...
            <Link to={salesPath(true)} className={`rounded-xl px-4 py-3 text-sm font-bold ${showArchived ? 'bg-brand-900 text-white' : 'bg-white text-brand-900'}`}>Pacas archivadas</Link>
          </nav>
          ...
          <div className="mb-3">
            <h2 id="sales-list-title" className="text-lg font-extrabold text-slate-900">
              {baleId ? `Pedidos de ${selectedBale?.code ?? 'la paca seleccionada'}` : showArchived ? 'Historial de pacas archivadas' : 'Pedidos de pacas vigentes'}
            </h2>
