  ...
  return (
    <div>
      <PageHeader
        eyebrow="Existencias"
        title="Inventario"
        description={`Una vista clara de ${terms.inventoryUnitPluralLower} disponibles, agrupados por categoría.`}
        backTo="/mas"
      />
        ...
        <section className="max-w-3xl rounded-3xl bg-brand-900 p-5 text-white shadow-lg shadow-brand-900/15">
          ...
          <div className="mt-2 flex items-end justify-between gap-4">
            ...
            <span className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-brand-100">
              ...
              {terms.inventoryUnitPluralLower} en existencia
            </span>
          </div>
        </section>
            ...
            {visibleCategories.map((category, index) => {
              ...
              return <div key={category.id} className={index > 0 ? 'border-t border-slate-100' : ''}>
                <div className="flex min-h-14 items-center justify-between gap-4 px-5 py-3">
                  ...
                  <p className="shrink-0 text-sm font-extrabold text-slate-900">{category.availablePieces} <span className="font-medium text-slate-400">{terms.inventoryUnitPluralLower}</span></p>
                </div>
