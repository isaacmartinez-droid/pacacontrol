...
function NewSalePage() {
  ...
  async function handleSubmit(event) {
    ...
    if (calculatedItems.some((item) => !item.category || !item.inventory || item.inventory.isActive === false)) return setFormError(`Selecciona la categoría y ${terms.purchaseSingularLower} activa de cada artículo.`)
    if (new Set(calculatedItems.map((item) => item.baleInventoryId)).size !== calculatedItems.length) return setFormError(`No repitas la misma categoría y ${terms.purchaseSingularLower}; agrega sus precios dentro del mismo artículo.`)
    for (const item of calculatedItems) {
      ...
      if (item.quantity > item.inventory.availablePieces) return setFormError(`${item.inventory.baleCode} solo tiene ${item.inventory.availablePieces} ${terms.inventoryUnitPluralLower} disponibles de ${item.category.name}.`)
    }
  ...
  if (completedSale) return <div><PageHeader eyebrow="Salida de inventario" title="Registrar venta" backTo="/ventas" /><div className="page-content py-6"><SaleConfirmation sale={completedSale} terms={terms} onAgain={() => { setCompletedSale(null); setForm(initialSaleForm(data.settings)) }} /></div></div>
  ...
  return <div>
    ...
    <div className="page-content py-6 md:py-8">
      {!isLoading && !error && !hasInventory ? <EmptyState icon={PackageX} title="No hay inventario disponible para vender" description={`Registra primero ${terms.purchaseSingularLower} con inventario.`} action={{ to: '/pacas/nueva', label: `Registrar ${terms.purchaseSingularLower}` }} /> :
          ...
          <section className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100 sm:p-6">
            ...
            <div className="mt-5 space-y-4">{calculatedItems.map((item, itemIndex) => {
              ...
              return <article key={item.id} className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label="Categoría"><select value={item.categoryId} onChange={(event) => updateItem(item.id, 'categoryId', event.target.value)} className="sale-input">{categories.map((category) => <option key={category.id} value={category.id}>{category.name} · {category.availablePieces} disponibles</option>)}</select></Field><Field label={`${terms.purchaseSingular} de origen`}><select required value={item.baleInventoryId} onChange={(event) => updateItem(item.id, 'baleInventoryId', event.target.value)} className="sale-input"><option value="">Selecciona {terms.purchaseSingularLower}</option>{baleOptions.map((inventory) => <option key={inventory.id} value={inventory.id}>{inventory.baleCode} · {inventory.availablePieces} disponibles · sugerido {formatCurrency(inventory.recommendedUnitPrice)}</option>)}</select></Field></div>
                ...
                <div className="mt-2 space-y-2">{item.priceLines.map((line, lineIndex) => {
                  ...
                  return <div key={line.id} className={`rounded-xl border p-3 ${warning ? belowCost ? 'border-red-200 bg-red-50/40' : 'border-amber-200 bg-amber-50/40' : 'border-slate-100 bg-slate-50/50'}`}><div className="grid items-end gap-2 sm:grid-cols-[0.55fr_1fr_0.75fr_auto]"><Field label={`Cantidad ${lineIndex + 1}`}><input type="number" min="1" value={line.quantity} onChange={(event) => updatePriceLine(item.id, line.id, 'quantity', event.target.value)} className="sale-input" /></Field><MoneyField label={`Precio por ${terms.inventoryUnitSingularLower}`} value={line.unitPrice} min="1" onChange={(value) => updatePriceLine(item.id, line.id, 'unitPrice', value)} /><div className="rounded-xl bg-white p-3 text-right"><p className="text-[0.65rem] text-slate-500">Subtotal</p><b className="text-sm">{formatCurrency(line.quantity * line.unitPrice)}</b></div>{item.priceLines.length > 1 && <button type="button" onClick={() => removePriceLine(item.id, line.id)} className="mb-1 rounded-lg p-2 text-coral-600" aria-label="Quitar precio"><Trash2 size={17} /></button>}</div>{warning && <p className={`mt-2 flex items-center gap-1 text-xs font-bold ${belowCost ? 'text-red-700' : 'text-amber-800'}`}><AlertTriangle size={14} />{belowCost ? 'Este precio está debajo del costo.' : 'Este precio está debajo del recomendado.'}</p>}</div>
                })}</div>
              </article>
            })}</div>
          </section>

          <FormSection icon={Users} title="Cliente" description="También puedes asociar un cliente que retirará su compra en el punto de venta."><Field label="Cliente"><select value={form.customerId} onChange={(event) => update('customerId', event.target.value)} className="sale-input"><option value="walk-in">Venta de mostrador</option><option value="new">+ Crear cliente nuevo</option>{data.customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></Field>{form.customerId === 'new' && <div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Nombre completo"><input value={form.newCustomerName} onChange={(event) => update('newCustomerName', event.target.value)} className="sale-input" /></Field><Field label="Teléfono"><input type="tel" value={form.newCustomerPhone} onChange={(event) => update('newCustomerPhone', event.target.value)} className="sale-input" /></Field></div>}</FormSection>

          <FormSection icon={hasDelivery ? Truck : Store} title="Forma de entrega" description="El delivery solo se solicita cuando realmente habrá un envío."><div className="grid grid-cols-2 gap-2"><ChoiceButton selected={!hasDelivery} onClick={() => update('fulfillmentMethod', 'pickup')} icon={Store} label="Retiro en punto de venta" /><ChoiceButton selected={hasDelivery} onClick={() => update('fulfillmentMethod', 'delivery')} icon={Truck} label="Enviar al cliente" /></div>{hasDelivery && <div className="mt-4 grid gap-4 sm:grid-cols-2"><MoneyField label="Costo real del delivery" min="1" value={form.deliveryCost} onChange={(value) => update('deliveryCost', value)} required /><MoneyField label="Cobro de delivery al cliente" value={form.deliveryCharge} onChange={(value) => update('deliveryCharge', value)} /></div>}</FormSection>
