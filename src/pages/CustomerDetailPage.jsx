...
function SaleHistory({ sale, method, onMethod, onComplete, saving }) {
  ...
  return <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-extrabold text-slate-900">{sale.pieces} {sale.pieces === 1 ? 'pieza' : 'piezas'} · {sale.baleCodes.join(', ') || 'Paca'}</p><p className="mt-1 inline-flex items-center gap-1 text-xs text-slate-500"><Clock3 size={13} />{dateTime(sale.soldAt)}</p></div><div className="flex items-center gap-2">{!sale.hasArchivedInventory && <Link to={`/ventas/${sale.id}/editar`} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-slate-200 px-2.5 text-xs font-extrabold text-brand-800"><Pencil size={13} />Editar piezas y total</Link>}<span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${sale.balance > 0 ? 'bg-amber-50 text-amber-800' : 'bg-emerald-50 text-emerald-800'}`}>{sale.balance > 0 ? `Debe ${formatCurrency(sale.balance)}` : 'Pagado'}</span></div></div>
    ...
    {sale.items.length > 0 && <div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Artículos individuales</p><div className="mt-3 space-y-3">{sale.items.map((item, index) => { const content = <><span className="text-slate-700"><b>{index + 1}. {item.categoryName}</b><small className="mt-0.5 block text-slate-500">{item.baleCode} · {item.quantity} {item.quantity === 1 ? 'pieza' : 'piezas'} × {formatCurrency(item.unitPrice)}</small></span><span className="text-right"><b className="text-slate-900">{formatCurrency(item.quantity * item.unitPrice)}</b>{item.recommendedUnitPrice > 0 && <small className="block text-slate-500">{sale.hasArchivedInventory ? 'Paca archivada: solo consulta' : 'Tocar para editar piezas y precio'}</small>}</span></>; return sale.hasArchivedInventory ? <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3 text-sm last:border-0">{content}</div> : <Link key={item.id} to={`/ventas/${sale.id}/editar`} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3 text-sm transition hover:text-brand-800 last:border-0 last:pb-0">{content}</Link> })}</div></div>}
