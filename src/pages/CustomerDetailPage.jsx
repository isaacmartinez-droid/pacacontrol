...
function SaleHistory({ sale, method, onMethod, onComplete, saving }) {
  ...
  return <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-extrabold text-slate-900">{sale.pieces} {sale.pieces === 1 ? 'pieza' : 'piezas'} · {sale.baleCodes.join(', ') || 'Paca'}</p><p className="mt-1 inline-flex items-center gap-1 text-xs text-slate-500"><Clock3 size={13} />{dateTime(sale.soldAt)}</p></div><div className="flex items-center gap-2">{sale.deliveryStatus !== 'delivered' && <Link to={`/ventas/${sale.id}/editar`} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-slate-200 px-2.5 text-xs font-extrabold text-brand-800"><Pencil size={13} />Editar pedido</Link>}<span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${sale.balance > 0 ? 'bg-amber-50 text-amber-800' : 'bg-emerald-50 text-emerald-800'}`}>{sale.balance > 0 ? `Debe ${formatCurrency(sale.balance)}` : 'Pagado'}</span></div></div>
