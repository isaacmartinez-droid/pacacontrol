...
const dateTime = (value) => value ? new Date(value).toLocaleString('es-NI', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'America/Managua' }) : '—'
...
function SaleHistory({ sale, terms, method, onMethod, onComplete, saving }) {
  ...
  return <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-extrabold text-slate-900">{sale.pieces} {inventoryQuantityLabel(sale.pieces, terms)} · {sale.baleCodes.join(', ') || terms.purchaseSingular}</p><p className="mt-1 inline-flex items-center gap-1 text-xs text-slate-500"><Clock3 size={13} />{dateTime(sale.soldAt)}</p></div><div className="flex items-center gap-2">{!sale.hasArchivedInventory && <Link to={`/ventas/${sale.id}/editar`} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-slate-200 px-2.5 text-xs font-extrabold text-brand-800"><Pencil size={13} />Editar artículos y total</Link>}<span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${sale.balance > 0 ? 'bg-amber-50 text-amber-800' : 'bg-emerald-50 text-emerald-800'}`}>{sale.balance > 0 ? `Debe ${formatCurrency(sale.balance)}` : 'Pagado'}</span></div></div>
    <dl className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5"><Detail label={terms.inventoryUnitPlural} value={formatCurrency(sale.merchandiseTotal)} /><Detail label="Entrega" value={sale.fulfillmentMethod === 'delivery' ? 'Envío' : 'Retiro en punto de venta'} /><Detail label="Cobro delivery" value={formatCurrency(sale.deliveryCharge)} /><Detail label="Costo delivery" value={formatCurrency(sale.deliveryCost)} /><Detail label={`Costo de ${terms.inventoryUnitPluralLower}`} value={formatCurrency(sale.estimatedMerchandiseCost)} /></dl>
    {sale.items.length > 0 && <div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Artículos individuales</p><div className="mt-3 space-y-3">{sale.items.map((item, index) => { const content = <><span className="text-slate-700"><b>{index + 1}. {item.categoryName}</b><small className="mt-0.5 block text-slate-500">{item.baleCode} · {item.quantity} {inventoryQuantityLabel(item.quantity, terms)} × {formatCurrency(item.unitPrice)}</small></span><span className="text-right"><b className="text-slate-900">{formatCurrency(item.quantity * item.unitPrice)}</b>{item.recommendedUnitPrice > 0 && <small className="block text-slate-500">{sale.hasArchivedInventory ? 'Registro en archivo: solo consulta' : 'Tocar para editar cantidad y precio'}</small>}</span></>; return sale.hasArchivedInventory ? <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3 text-sm last:border-0">{content}</div> : <Link key={item.id} to={`/ventas/${sale.id}/editar`} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3 text-sm transition hover:text-brand-800 last:border-0 last:pb-0">{content}</Link> })}</div></div>}
    <div className="mt-4 grid gap-3 sm:grid-cols-2"><Payment title="Primer pago" amount={sale.firstPaymentAmount} method={sale.firstPaymentMethod} at={sale.firstPaymentAt} /><Payment title="Segundo pago" amount={sale.secondPaymentAmount} method={sale.secondPaymentMethod} at={sale.secondPaymentAt} />{sale.additionalPayments?.map((payment, index) => <Payment key={payment.id} title={'Pago adicional ' + (index + 1)} amount={payment.amount} method={payment.method} at={payment.paidAt} />)}</div>
