...
import { CircleDollarSign, Clock3, Phone, ReceiptText, TrendingDown, TrendingUp } from 'lucide-react'
  ...
  return <div>
    ...
    <div className="page-content space-y-5 py-5 md:py-8">
      <section className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
        <div className="flex flex-wrap items-center justify-between gap-3"><div>{customer.phone ? <a href={`tel:${customer.phone.replaceAll(' ', '')}`} className="inline-flex items-center gap-2 font-bold text-brand-700"><Phone size={17} />{customer.phone}</a> : <span className="text-sm text-slate-500">Sin teléfono registrado</span>}</div>{customer.priority && <span className="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-extrabold text-amber-700">Cliente prioritario</span>}</div>
...
function SaleHistory({ sale, method, onMethod, onComplete, saving }) {
  ...
  return <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
    ...
    <dl className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Prendas" value={formatCurrency(sale.merchandiseTotal)} /><Detail label="Cobro delivery" value={formatCurrency(sale.deliveryCharge)} /><Detail label="Costo delivery" value={formatCurrency(sale.deliveryCost)} /><Detail label="Costo prendas" value={formatCurrency(sale.estimatedMerchandiseCost)} /></dl>
    {sale.priceLines.length > 0 && <div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Desglose de precios</p><div className="mt-2 space-y-2">{sale.priceLines.map((line, index) => <div key={`${line.unitPrice}-${index}`} className="flex flex-wrap items-center justify-between gap-2 text-sm"><span className="text-slate-700">{line.quantity} {line.quantity === 1 ? 'pieza' : 'piezas'} × <b>{formatCurrency(line.unitPrice)}</b></span><span className="text-right"><b className="text-slate-900">{formatCurrency(line.quantity * line.unitPrice)}</b>{line.recommendedUnitPrice > 0 && <small className="ml-2 text-slate-500">Sugerido: {formatCurrency(line.recommendedUnitPrice)}</small>}</span></div>)}</div></div>}
