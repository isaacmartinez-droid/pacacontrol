...
function SaleHistory({ sale, method, onMethod, onComplete, saving }) {
  ...
  return <article className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100">
    ...
    <dl className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Prendas" value={formatCurrency(sale.merchandiseTotal)} /><Detail label="Cobro delivery" value={formatCurrency(sale.deliveryCharge)} /><Detail label="Costo delivery" value={formatCurrency(sale.deliveryCost)} /><Detail label="Costo prendas" value={formatCurrency(sale.estimatedMerchandiseCost)} />{sale.unitPrice > 0 && <Detail label="Precio cobrado por pieza" value={formatCurrency(sale.unitPrice)} />}{sale.recommendedUnitPrice > 0 && <Detail label="Recomendado al vender" value={formatCurrency(sale.recommendedUnitPrice)} />}</dl>
