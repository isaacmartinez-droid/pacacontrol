...
export default function EditBalePage() {
  ...
  async function submit(event) {
    ...
    if (bale.isArchived) return setMessage(`Reactiva esta ${terms.purchaseSingularLower} antes de corregir sus datos.`)
    ...
    if (values.inventoryLines.some((line) => !Number.isInteger(line.receivedPieces) || line.receivedPieces < line.minimumPieces)) return setMessage(`Una cantidad no puede ser menor que sus ${terms.inventoryUnitPluralLower} vendidos y dañados.`)
    ...
    try {
      ...
      setMessage(error.message || `No fue posible editar la ${terms.purchaseSingularLower}.`)
    } finally {
      setIsSaving(false)
    }
  }
  ...
  async function changeArchive() {
    ...
    try {
      ...
      setMessage(error.message || `No fue posible cambiar el estado de archivo de la ${terms.purchaseSingularLower}.`)
    } finally {
      setIsDeleting(false)
    }
  }

  if (!bale && !isLoading) return <div><PageHeader title={`${terms.purchaseSingular} no encontrada`} backTo="/pacas" /><div className="page-content py-6"><EmptyState icon={CircleAlert} title="Este registro no está disponible" description="Puede no pertenecer a esta cuenta." /></div></div>
  if (!form) return <div><PageHeader title={`Cargando ${terms.purchaseSingularLower}…`} backTo="/pacas" /></div>
    ...
    <div className="page-content py-6 md:py-8">
      <form onSubmit={submit} className="max-w-4xl space-y-5">
        <section className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100 sm:p-6">
          <h2 className="text-lg font-extrabold text-slate-900">Datos de la {terms.purchaseSingularLower}</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <Money label={`Ganancia deseada para toda la ${terms.purchaseSingularLower}`} value={form.targetProfitAmount} onChange={(value) => update('targetProfitAmount', value)} required />
          </div>
        ...
        <section className="rounded-3xl bg-white p-5 shadow-soft ring-1 ring-slate-100 sm:p-6">
          <h2 className="text-lg font-extrabold text-slate-900">{terms.inventoryUnitPlural} por categoría</h2>
          ...
          <div className="mt-5 space-y-3">{form.inventoryLines.map((line) => <div key={line.id} className="rounded-2xl bg-slate-50 p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <Field label={`${terms.inventoryUnitPlural} recibidos`}><input type="number" min={line.minimumPieces} step="1" value={line.receivedPieces} onChange={(event) => updateInventory(line.id, 'receivedPieces', event.target.value)} className="sale-input" /></Field>
