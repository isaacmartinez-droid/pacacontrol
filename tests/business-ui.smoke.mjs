...
try {
  ...
  await page.getByRole('heading', { name: 'Conoce cómo funciona ControlShop', exact: true }).waitFor()
  for (let step = 0; step < 5; step += 1) await page.getByRole('button', { name: 'Siguiente', exact: true }).click()
  await page.getByRole('button', { name: 'Ver mi guía de primeros pasos', exact: true }).click()
  await page.getByRole('heading', { name: 'Empieza con lo importante', exact: true }).waitFor()
  await page.getByText('Registra una compra', { exact: true }).waitFor()
  await page.getByText('Registra un pedido o venta', { exact: true }).waitFor()
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), true)
  ...
  await page.getByRole('link', { name: 'Ir a mi resumen', exact: true }).click()
  await page.getByRole('heading', { name: 'Tu resumen', exact: true }).waitFor()
  await page.goto(origin + '/primeros-pasos')
  await page.getByRole('link', { name: 'Ver la presentación nuevamente', exact: true }).click()
  await page.getByRole('heading', { name: 'Conoce cómo funciona ControlShop', exact: true }).waitFor()
  ...
  await page.goto(origin + '/recorrido-inicial')
  await page.getByRole('heading', { name: 'Conoce cómo funciona ControlShop', exact: true }).waitFor()
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), true)
  await page.goto(origin + '/primeros-pasos')
  await page.getByRole('heading', { name: 'Empieza con lo importante', exact: true }).waitFor()
  ...
  await page.getByRole('heading', { name: 'Conoce cómo funciona ControlShop', exact: true }).waitFor()
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), true)
  console.log('OK: presentación y guía inicial se adaptan a móvil y tablet sin desplazamiento horizontal.')
  assert.deepEqual(errors, [])
