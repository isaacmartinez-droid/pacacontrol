...
export default async function handler(req, res) {
  ...
  if (!pushConfigured()) return res.status(200).json({ configured: false, reason: 'server-configuration' })
  try {
    const { data, error } = await getPushAdmin().from('push_dispatch_state').select('last_run_at').eq('id', true).single()
    if (error) return res.status(200).json({ configured: false, reason: 'database-configuration' })
